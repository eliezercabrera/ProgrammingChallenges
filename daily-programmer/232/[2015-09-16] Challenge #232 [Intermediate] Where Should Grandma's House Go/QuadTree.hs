...
data Coordinate = Coordinate !Distance !Distance deriving (Eq, Ord, Read, Show)

euclideanDistance :: Coordinate -> Coordinate -> Distance
euclideanDistance (Coordinate x1 y1) (Coordinate x2 y2) = sqrt $ (x1 - x2)^(2 :: Int) + (y1 - y2)^(2 :: Int)

euclideanDistanceApprox :: Coordinate -> Coordinate -> Distance
euclideanDistanceApprox (Coordinate x1 y1) (Coordinate x2 y2) = (x1 - x2)^(2 :: Int) + (y1 - y2)^(2 :: Int)
...
comparePoints (Coordinate x1 y1) (Coordinate x2 y2) = case (x1 <= x2, y1 <= y2) of
                                                        (True , True ) -> Q1
                                                        (True , False) -> Q4
                                                        (False, True ) -> Q2
                                                        (False, False) -> Q3
...
insertPoint size c (Leaf ce@(Coordinate x y) hd s)
                                  | Set.size s < size = Leaf ce hd (Set.insert c s)
                                  | otherwise = insertPoint size c branch
                                      where e1 = empty (Coordinate (x + 0.5*hd) (y + 0.5*hd)) (0.5*hd)
                                            e2 = empty (Coordinate (x - 0.5*hd) (y + 0.5*hd)) (0.5*hd)
                                            e3 = empty (Coordinate (x - 0.5*hd) (y - 0.5*hd)) (0.5*hd)
                                            e4 = empty (Coordinate (x + 0.5*hd) (y - 0.5*hd)) (0.5*hd)
...
insertAndFindClosestPoint :: Coordinate -> QuadTreeInternal -> (Coordinate, QuadTreeInternal)
insertPoint size c (Leaf ce@(Coordinate x y) hd s)
                                  | Set.size s < size = Leaf ce hd (Set.insert c s)
                                  | otherwise = insertPoint size c branch
                                      where e1 = empty (Coordinate (x + 0.5*hd) (y + 0.5*hd)) (0.5*hd)
...
deletePoint size c (Leaf ce hd s) = Leaf ce hd (Set.delete c s)
...
isInRange c@(Coordinate x1 y1) d (Leaf ce@(Coordinate x2 y2) hd _)
...
isInRange c@(Coordinate x1 y1) d (Branch ce@(Coordinate x2 y2) hd _ _ _ _ _)
        = case comparePoints c ce of
            Q1 -> x1 + d >= x2 - hd && y1 + d >= y2 - hd
            Q2 -> x1 - d <= x2 + hd && y1 + d >= y2 - hd
            Q3 -> x1 - d <= x2 + hd && y1 - d <= y2 + hd
